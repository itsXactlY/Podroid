...
object AvfCapabilities {
    ...
    /* The platform used to launch, for a bare custom-image VM, a crosvm build
     * without the `net` cargo feature — so virtmgr's `--net tap-fd=N` was not a
     * valid argument and every guest died with exit 35 moments after "booting".
     * Podroid no longer hits that path: see AvfReflect.setAudioConfig for the
     * one-line reason, and why it is fixed on our side rather than the
     * platform's. */
